...
def lnk_file_after_header(lnk_file_data):
    ...
    after_header_list.append(Volume_Label.replace("\x00\x00", "").replace("\x00", ""))
    ...
    if lnk_info_flags[0] == 1:
        ...
        # LocalBasePath = lnk_localbasepath.decode('utf-8').replace("\x00\x00", "").replace("\x00", "")
        LocalBasePath = lnk_localbasepath.decode('ascii').replace("\x00\x00", "").replace("\x00", "")
...
def destlist_data(destlist_file_data):
    ...
    # Number of add/delete actions – Increments as entries are added.  Also increments as individual entries are deleted.
    destlist_add_delete = destlist_file_data[24:32]
    ...
    # destlist_netbiosname = destlist_netbiosname.decode('utf-8').replace("\x00", "").replace("\x00", "")
    destlist_netbiosname = destlist_netbiosname.decode('ascii').replace("\x00", "")
    ...
    for entry in range(destlist_totalentries[0] - 1):
        ...
        if destlist_entryidnumber[0] > 1:
            ...
            destlist_entry_netbiosname = destlist_netbiosname.decode('ascii').replace("\x00", "")
